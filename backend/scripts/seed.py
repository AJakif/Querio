"""Seed the database with the Olist Brazilian E-Commerce dataset schema and deterministic sample data.

Usage:
    python scripts/seed.py

This script creates the full Olist schema (9 tables) with foreign key relationships
and populates them with reproducible, arithmetically coherent sample data.

Data characteristics:
  - 1,000 customers across Brazilian states
  - 5,000 orders with realistic status distribution
  - 50 products in ~30 categories with Portuguese names
  - 50 sellers
  - ~5,000 order_items, order_payments, and order_reviews rows
  - Geolocation data covering major Brazilian cities
...
Arithmetic coherence guarantees:
  - order_payments.payment_value == SUM(order_items.price + freight_value) per order
  - dates are consistent: purchase < approval < delivery
  - only 'delivered' orders have delivery dates populated
"""
...
def _generate_orders(
    ...
    for i in range(1, count + 1):
        ...
        installments = rng.choices([1, 2, 3, 4, 5, 6, 8, 10, 12], weights=[30, 20, 15, 10, 8, 5, 5, 4, 3], k=1)[0]
        all_payments.append({
            "order_id": oid,
...
def main():
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute(_build_insert("customers", customers))
            conn.commit()

            print("Seeding geolocation...")
            cur.execute(_build_insert("geolocation", geolocation))
            conn.commit()

            print("Seeding products...")
            cur.execute(_build_insert("products", products))
            conn.commit()

            print("Seeding sellers...")
            cur.execute(_build_insert("sellers", sellers))
            conn.commit()

            print("Seeding orders...")
            cur.execute(_build_insert("orders", orders))
            conn.commit()

            print("Seeding order_items...")
            for batch_start in range(0, len(order_items), 500):
                batch = order_items[batch_start:batch_start + 500]
                cur.execute(_build_insert("order_items", batch))
                conn.commit()

            print("Seeding order_payments...")
            cur.execute(_build_insert("order_payments", order_payments))
            conn.commit()

            print("Seeding order_reviews...")
            for batch_start in range(0, len(order_reviews), 500):
                batch = order_reviews[batch_start:batch_start + 500]
                cur.execute(_build_insert("order_reviews", batch))
                conn.commit()

            print("Seeding product_categories...")
            cur.execute(_build_insert("product_categories", categories))
            conn.commit()
        ...
        print(f"Database seeded successfully: {total_rows} rows across 9 tables")
